...
double mSecsTotal_12 = 0.0;
pdi_reproduce_t_12 infoThread_12[NRO_HILOS];

int main(int argc, char* argv[]) //Comentado para que funcione el ejercicio 8.16
{
	double aux = 0.0;
	...
	for (size_t i = 0; i < NRO_HILOS; i++)
	{
		...
		p->h = CreateThread(NULL, 0, Reproduce_12, p, 0, &p->id);
	...
	while (TRUE)
	{
		for (size_t i = 0; i < NRO_HILOS; i++)
		{
			aux = aux + infoThread_12[i]->sec;
		}
		mSecsTotal_12 = aux;
		aux = 0.0;
		if (mSecsTotal_12 - (int)(mSecsTotal_12 / 5) * 5 == 0) printf("%.2f\n", mSecsTotal_12);
	}
	//miniMenu();
...
void miniMenu_12() {
	...
	while (TRUE) {
		...
		{
			CheckError(FALSE == ReleaseSemaphore(infoThread_12[cmd]->evSem, 1, NULL), "No se pudo activar semaforo de terminacion", 1);
			WaitForSingleObject(infoThread_12[cmd]->h, INFINITE);
		}
	}
}