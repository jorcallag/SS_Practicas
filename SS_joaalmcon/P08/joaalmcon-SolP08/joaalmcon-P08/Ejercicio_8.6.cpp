...
{	
	...
	semCuenta = CreateSemaphore(NULL, 1, 1, NULL);
	CheckError(NULL == semCuenta, "No se pudo crear el semaforo de cuenta", 1);
	...
	for (size_t i = 0; i < NRO_VENTANAS; i++)
	{
		...
		p->semFin = CreateSemaphore(NULL, 0, 1, NULL);
		CheckError(NULL == p->semFin, "No se pudo crear el evento de terminación", 1);
	...
	for (int i = 0; i < NRO_VENTANAS; i++) {
		CheckError(WAIT_OBJECT_0 != WaitForSingleObject(infoHilos[i]->h, INFINITE), "Error al bloquear", 1);
		CloseHandle(infoHilos[i]->h);
		delete infoHilos[i];
	}
	CloseHandle(semCuenta);
...
DWORD WINAPI Reproduce(_In_ LPVOID lpParameter) {
	...
	do
	{
		...
		CheckError(0 != WaitForSingleObject(semCuenta, INFINITE), "Error en el bloqueo", 1);
		
		if (frameA > ahora)
			aux = 1;
		else if (frameA = ahora)
			aux = gap;
		else {
			frameA = ahora;
			aux = gap;
		...
		Sleep((int)aux);
		CheckError(FALSE == ReleaseSemaphore(semCuenta, 1, NULL), "No se pudo desbloquear el objeto", 1);
		...
		DWORD res =	WaitForSingleObject(pdi->semFin, 0);
...
void miniMenu() {
	...
	while (TRUE) {
		...
		if (cmd < 0)
			...
			ReleaseSemaphore(infoHilos[cmd]->semFin, 1, NULL);

			WaitForSingleObject(infoHilos[cmd]->h, INFINITE);

			CheckError(NULL == infoHilos[cmd]->h, "No se pudo crear el semaforo de terminación", 1);
		}
