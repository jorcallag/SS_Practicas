...
void miniMenu_Eje8();
DWORD WINAPI reproduce_h_Eje8(LPVOID lpParameter);

HANDLE hReproduce_Eje8;

/*int main(int argc, char *argv[]) //Comentado para que ejecute P07_Eje11_jorcallag
{

	setlocale(LC_ALL, "Spanish");
	CheckError(argc != 2, "Debe haber al menos 1 argumentos (nombre archivo de la imagen)", 1);

	hReproduce_Eje8 = CreateThread(NULL, 0, reproduce_h_Eje8, argv[1], 0, NULL);
	CheckError(NULL == hReproduce_Eje8, "\n\n\ERROR al crear el hilo de reproduccionn\n", 1);
...
DWORD WINAPI reproduce_h_Eje8(LPVOID lpParameter) {
	...
	do
	{
		...
		AGris_2x(imgOriginal, imgResultado);
		//Sleep(100);
